...
The mi	p.filters = append(p.filters, m.f) // a measurement is also a filter
cro configuration language is quite simple to parse anyway.
...
func (p *Parser) addFilter(name string, f filter) (filter, error) {
	old := p.n2f[name]
	if old != nil {
		return nil, p.syntaxError(fmt.Sprintf("filter %q already declared", name))
	}
	p.n2f[name] = f
	p.f2n[f] = name
	return f, nil
...
func (p *Parser) Parse() error {
	for {
		...
		case tTLeftArrow: // [name <-] filter
			...
			f, err = p.addFilter(name, f) // this filter is named, store for later retrieval
...
func (p *Parser) parseIdentifierList() ([]string, error) {
	...
	for {
		...
		il = append(il, lit) // TODO lowercase the identifiers?
		tok, lit = p.scanIgnoreWhitespace()
